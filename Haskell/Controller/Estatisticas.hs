...
generoMaisLido livros =
    let generos = listaGeneros livros  -- Obtém a lista de gêneros
        contagemGeneros = map (\gen -> (gen, generoNumLeituras livros gen)) generos  -- Conta quantas vezes cada gênero aparece
        generoMaisLido = fst $ maximumBy (compare `on` snd) contagemGeneros  -- Encontra o gênero com maior contagem
    in generoMaisLido
...
autorMaisLido livros =
    let autores = listaAutores livros  -- Obtém a lista de autores
        contagemAutores = map (\aut -> (aut, autorNumLeituras livros aut)) autores  -- Conta quantas vezes cada autor aparece
        autorMaisLido = fst $ maximumBy (compare `on` snd) contagemAutores  -- Encontra o autor com maior contagem
    in autorMaisLido   

-- Função que retorna o primeiro livro de melhor avaliação
melhorLivro :: [Livro] -> Livro
melhorLivro livros =
    let sortedLivros = sortBy (flip (comparing avaliacao)) livros  -- Ordena os livros pela avaliação em ordem decrescente
    in head sortedLivros  -- Seleciona o primeiro livro da lista ordenada
